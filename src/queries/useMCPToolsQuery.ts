...
const fetchMCPTools = async (endpoint: string): Promise<unknown> => {
    ...
    try {
        ...
        if (error.name === 'TypeError' || error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
        ...
        else if (error.response?.status === 400 || error.response?.status === 404 || error.response?.status === 405) {
